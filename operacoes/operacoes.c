...
// Verifica se o campo informado e de tamanho variavel
static int campoString(char *campo) {
    return strcmp(campo, "nomeEstacao") == 0 || strcmp(campo, "nomeLinha") == 0;
}


// Le os criterios de uma busca no mesmo formato usado pela funcionalidade 3
    ...
    for (int i = 0; i < qtdCriterios; i++) {
        ...
        if (campoString(criterios[i].nomeCampo)) {
            ...
            criterios[i].isNulo = (strcmp(criterios[i].valorStr, "") == 0 ||
                                   strcmp(criterios[i].valorStr, "NULO") == 0) ? 1 : 0;
        } else {
            char temp[50];
            scanf("%s", temp);
...
// Retorna 1 se o registro satisfaz todos os criterios informados
...
// Busca sequencialmente no arquivo de dados usando os criterios informados
...
// Busca pelo indice arvore-B e valida os demais criterios no registro encontrado
...
// Funcao que cria o indice arvore-B a partir do arquivo de dados
...
// Funcao Select Where que usa arvore-B quando houver criterio por codEstacao
